...
The initial shared files originate from the currently approved MQTTSuite TikZ
system:

- `landingpages-figure-system.tex` — shared base TikZ vocabulary, helpers, node
  families, typography, ports, connector semantics, and publication rules;
...
New technical figures should load the shared system with these stable input
names:
...
- MQTTSuite can be migrated to the shared location in a later dedicated pass;
  only then should its duplicate local system files be removed or renamed.
